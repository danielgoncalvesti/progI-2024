...
print(f"Ex 1: Número de palavras: {len(lista_palavras)}")
...
for palavra in lista_palavras:
    if n > palavra:
        n = palavra
print(f"Ex 2: Palavra Mais Longa: {n}")

# Ex 3. Inverta a ordem das palavras no texto e imprima o resultado. Use 02-for ou 01-while.
# Exemplo: "Olá, mundo!" -> "mundo! Olá,"
indice = len(lista_palavras) - 1
ordem_trocada = []
while indice > 0:
    ordem_trocada.append(lista_palavras[indice])
    indice = indice - 1
print(f"Ex 3: Invertendo a Ordem das Palavras:{ordem_trocada} ")
...
while indice > 0:
    for x in texto[indice]:
        invertido += x
        indice -= 1
print(f"Ex 4: Invertendo a Ordem dos Caracteres: {invertido}")
